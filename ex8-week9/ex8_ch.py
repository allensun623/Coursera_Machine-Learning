#https://github.com/AceCoooool/ML-Andrew-Ng/blob/master/Exercise8/ex8.py
import numpy as np
...
def select_threshold(yval, pval):
    ...
    for epsilon in np.arange(np.min(pval),np.max(pval), stepsize):
        pos = np.where(pval < epsilon)
        neg = np.where(pval >= epsilon)
        tp = np.count_nonzero(yval[pos] == 1)
        fp = np.count_nonzero(yval[pos] == 0)
        fn = np.count_nonzero(yval[neg] == 1)
