...
def map_feature(X):
    ...
    X1_X2 = X1 * X2
    X1_X2_square = X1 * X2_square
    X = np.stack([X0, X1, X2, X1_square, X2_square, X1_X2, X1_X2_square], axis=1)
    return X
...
def plot_decision_boundary(theta, x, y):
    ...
    for i in range(np.size(u, 0)):
        for j in range(np.size(v, 0)):
            z[i, j] = map_feature2(np.array([u[i]]), np.array([v[j]])).dot(theta)
    z = z.T
